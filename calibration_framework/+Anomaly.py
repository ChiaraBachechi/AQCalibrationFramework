...
class CalibrationAlgorithmFramework():
	...
	def loadDatasetFromDbAnomalyDetection(self):
        ...
        for f in self.features:
			...
			'= True THEN 0 ELSE ' + str(f) + ' END)/(CASE when sum(CASE WHEN a.' + pollutantName +\
        ...
        q="""
          select sensor.*, station.co,station.no,station.o3,station.no2
             ...
             select status.id_sensor_low_cost
                ...
                where id_anomaly_detection_algorithm=6) AS a ON (a.phenomenon_time_sensor_raw_observation=raw.phenomenon_time and a.id_sensor_low_cost_status=raw.id_sensor_low_cost_status)
                     ,(select ss1.id
    ...
    def createTrainingAndTestingDB(self):
        """
        ...
        #print (" test_labels columns: " + str(test_labels.columns))
        self.df_testAndTraining['train_features']=train_features
        self.df_testAndTraining['train_labels']=train_labels
